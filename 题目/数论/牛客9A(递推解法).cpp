//斐波那契K次幂和1： 
//对mod的性质无要求  本题mod并非质数  代入前2K+1项 套用普适版本线性递推模板即可
//复杂度O(k^2logn)
#include<bits/stdc++.h>
#define ll long long
using namespace std;
...
int main() {
    ...
    for (int i = 2; i <= 2 * m1 + 1;++i){
        fibo.push_back((fibo[i - 1] + fibo[i - 2])%mod);
    }
    for (int i = 0; i < fibo.size();i++){
        fibo[i] = bin(fibo[i], m1, mod);
    }
    for (int i = 1; i < fibo.size();++i){
        fibo[i] = (fibo[i - 1] + fibo[i]) % mod;
    }
    LinearRecurrence gp(fibo, mod, false);
    printf("%lld\n", gp.calc(n1));
    return 0;
}